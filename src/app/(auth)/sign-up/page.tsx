...
const SignIn = () => {
  ...
  const debounced = useDebounceCallback(setUsername, 300); //to check the username is available or not to reduce the server load
  const { toast } = useToast();
  const router = useRouter();
  //zod implemetation
  const form = useForm<z.infer<typeof signUpSchema>>({
    //useform will follow this schema
    resolver: zodResolver(signUpSchema), // the resolver requires a schema
    defaultValues: {
      username: "",
      email: "",
      password: "",
